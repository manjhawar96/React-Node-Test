...
const MeetingView = (props) => {
    ...
    const handleDelete = async () => {
        try {
          ...
          const result = await fetch(`/api/meeting/${id}`, {
            method: 'DELETE',
            headers: {
          ...
          if (result.status === 200) {
            ...
            toast.error(json?.error || 'Failed to delete meeting');
          }
        } catch (err) {
          console.error(err);
                ...
                {isLoding ?
                    <Flex justifyContent={'center'} alignItems={'center'} mb={30} width="100%" >
                        ...
                        <DrawerFooter>
                            ...
                            {access?.delete && <IconButton colorScheme='red' onClick={handleDelete} ml={3} borderRadius="10px" size="md" icon={<DeleteIcon />} />}
                        </DrawerFooter>
                    </>}
            </ModalContent>
        </Modal>
